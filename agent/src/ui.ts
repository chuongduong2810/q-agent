 ...
 * purple→indigo, custom frameless titlebar). No framework / no extra deps —
...
<style>
  *{box-sizing:border-box}
  ...
  @keyframes ring{0%{transform:scale(.5);opacity:.8}100%{transform:scale(1.4);opacity:0}}
  ...
  <div style="position:relative;z-index:2;flex:1;overflow-y:auto;padding:22px 22px 18px">
   <div style="max-width:420px;margin:0 auto">
    ...
    <div style="display:flex;align-items:flex-start;gap:14px;margin-bottom:26px">
      ...
      <div id="pill" style="display:flex;align-items:center;gap:7px;flex-shrink:0;padding:6px 12px;border-radius:20px">
    ...
    <div id="formView" style="display:none">
      <div style="animation:fadeUp .5s ease both;border-radius:22px;background:rgba(20,20,28,.55);backdrop-filter:blur(28px);border:1px solid rgba(255,255,255,.08);padding:24px;box-shadow:0 24px 60px -22px rgba(0,0,0,.7)">
        ...
        <div style="margin-bottom:22px">
          ...
          <div id="codeBox" style="display:flex;align-items:center;gap:10px;height:46px;padding:0 14px;border-radius:12px;background:#16161f;border:1px solid rgba(255,255,255,.1);transition:border-color .18s">
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#7a7a8c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><rect x="3" y="11" width="18" height="10" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
            <input id="code" inputmode="numeric" maxlength="6" placeholder="000000" style="flex:1;min-width:0;background:none;border:none;color:#ECECF1;font-size:15px;letter-spacing:.4em;font-family:'JetBrains Mono',monospace" />
          </div>
        </div>
    ...
    <div id="connView" style="display:none">
      <div style="animation:fadeUp .5s ease both">
        <div style="position:relative;overflow:hidden;border-radius:22px;background:linear-gradient(135deg,rgba(16,185,129,.12),rgba(20,20,28,.55));backdrop-filter:blur(28px);border:1px solid rgba(52,211,153,.28);padding:22px;box-shadow:0 24px 60px -22px rgba(0,0,0,.7);margin-bottom:14px">
          <div style="display:flex;align-items:center;gap:13px">
            <div style="position:relative;width:44px;height:44px;flex-shrink:0;display:flex;align-items:center;justify-content:center">
              <span style="position:absolute;inset:0;border-radius:50%;border:1.5px solid rgba(52,211,153,.5);animation:ring 2.4s ease-out infinite"></span>
              <span style="position:absolute;inset:0;border-radius:50%;border:1.5px solid rgba(52,211,153,.5);animation:ring 2.4s ease-out infinite 1.2s"></span>
              <span style="position:relative;width:44px;height:44px;border-radius:13px;background:linear-gradient(135deg,#10b981,#059669);display:flex;align-items:center;justify-content:center;box-shadow:0 0 26px -4px rgba(16,185,129,.8)"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6 9 17l-5-5"/></svg></span>
            </div>
            <div style="flex:1;min-width:0">
              <div style="font-size:16px;font-weight:800;letter-spacing:-.01em">This machine is live</div>
              <div style="font-size:12px;color:#9fd8c2;margin-top:3px">Paired · ready to run tests locally</div>
            </div>
            <button id="disconnect" style="flex-shrink:0;display:flex;align-items:center;gap:7px;padding:9px 14px;border-radius:11px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.12);cursor:pointer;font-family:inherit;font-size:12px;font-weight:600;color:#c7c7d4"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18.36 6.64A9 9 0 1 1 5.64 6.64M12 2v10"/></svg>Disconnect</button>
          </div>
          <div id="meta" style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:18px"></div>
        </div>

        <div style="border-radius:18px;background:rgba(20,20,28,.5);backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,.07);padding:18px;margin-bottom:14px">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
            <span style="font-size:12.5px;font-weight:700">Browser workers</span>
            <span style="font-size:10.5px;color:#7a7a8c">4 slots · idle</span>
          </div>
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:9px">
            <div style="display:flex;flex-direction:column;align-items:center;gap:8px;padding:13px 6px;border-radius:12px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06)"><div style="width:30px;height:30px;border-radius:9px;display:flex;align-items:center;justify-content:center;background:rgba(66,133,244,.2)"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="1.9"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3.2"/><path d="M12 9h8M8 13l-4 6M16 13l4 6"/></svg></div><span style="font-size:10px;font-weight:600;color:#c3c3d0">Chromium</span></div>
            <div style="display:flex;flex-direction:column;align-items:center;gap:8px;padding:13px 6px;border-radius:12px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06)"><div style="width:30px;height:30px;border-radius:9px;display:flex;align-items:center;justify-content:center;background:rgba(255,149,0,.2)"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="1.9"><circle cx="12" cy="12" r="9"/><path d="M12 3c4 2 5 6 3 9"/></svg></div><span style="font-size:10px;font-weight:600;color:#c3c3d0">Firefox</span></div>
            <div style="display:flex;flex-direction:column;align-items:center;gap:8px;padding:13px 6px;border-radius:12px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06)"><div style="width:30px;height:30px;border-radius:9px;display:flex;align-items:center;justify-content:center;background:rgba(52,211,153,.2)"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="1.9"><circle cx="12" cy="12" r="9"/><path d="M12 3v18M3 12h18"/></svg></div><span style="font-size:10px;font-weight:600;color:#c3c3d0">WebKit</span></div>
            <div style="display:flex;flex-direction:column;align-items:center;gap:8px;padding:13px 6px;border-radius:12px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06)"><div style="width:30px;height:30px;border-radius:9px;display:flex;align-items:center;justify-content:center;background:rgba(66,133,244,.2)"><svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="1.9"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="3.2"/><path d="M12 9h8M8 13l-4 6M16 13l4 6"/></svg></div><span style="font-size:10px;font-weight:600;color:#c3c3d0">Chromium</span></div>
          </div>
        </div>
        ...
        <div style="border-radius:18px;background:rgba(8,8,13,.72);border:1px solid rgba(255,255,255,.08);overflow:hidden">
          ...
          <div id="log" style="padding:13px 15px;font-family:'JetBrains Mono',monospace;font-size:11px;line-height:1.9;max-height:170px;overflow-y:auto"></div>
        </div>
      </div>
    </div>
...
// Electron window controls (exposed by the preload). Hidden in a plain browser.
...
function canConnect(){ return el("server").value.trim() && el("code").value.trim(); }
...
function timeStr(ts){ var d = new Date(ts); return d.toTimeString().slice(0,8); }
function logColor(t){
...
function renderConnected(s){
  ...
  var rows = [
    { k:"MACHINE", v:s.machine || "this machine", font:"inherit" },
    { k:"SESSION", v:"device #" + (s.deviceId==null?"?":s.deviceId), font:mono },
    { k:"SERVER", v:s.serverUrl || "\\u2014", font:mono },
    { k:"UPTIME", v:uptime(s.connectedSince), font:"inherit" }
  ];
  var html = "";
  for (var i=0;i<rows.length;i++){
    var r = rows[i];
    html += '<div style="padding:11px 13px;border-radius:12px;background:rgba(0,0,0,.22);border:1px solid rgba(255,255,255,.06)">'
      + '<div style="font-size:9.5px;font-weight:700;letter-spacing:.09em;color:#6c8c7e;margin-bottom:4px">' + r.k + '</div>'
      + '<div style="font-size:12.5px;font-weight:600;color:#dfece7;font-family:' + r.font + ';white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="' + r.v + '">' + r.v + '</div></div>';
  }
...
function showConnected(s){
  ...
  setPill("connected"); renderConnected(s);
...
el("code").addEventListener("input", function(){ el("code").value = el("code").value.replace(/[^0-9]/g,""); paintConnectBtn(); });
["server","code"].forEach(function(id){
  el(id).addEventListener("focus", function(){ el(id==="server"?"urlBox":"codeBox").style.borderColor = "rgba(139,92,246,.55)"; });
  el(id).addEventListener("blur", function(){ el(id==="server"?"urlBox":"codeBox").style.borderColor = "rgba(255,255,255,.1)"; });
});
...
el("connect").addEventListener("click", function(){
  ...
  fetch("/api/pair", { method:"POST", headers:{"Content-Type":"application/json"},
    body: JSON.stringify({ code: el("code").value.trim(), server: el("server").value.trim() }) })
    .then(function(r){ return r.json().then(function(j){ return { ok:r.ok, j:j }; }); })
    .then(function(res){
      if (!res.ok) throw new Error(res.j.error || "Pairing failed");
      refresh();
    })
