...
- **`knowledge.md` / `knowledge.json`** from `project-bootstrap` for terminology, roles, workflows,
  and existing automation patterns. This context is now richer and is injected into the
  generation prompt: the application base URL, real application routes/screens, real selectors,
  the auth/login flow, environments, business entities, configured test-account **roles** (not
  passwords), and reusable asset names. Preconditions and steps should reference these real values.
...
3. **Generate cases** — write each test case using `templates/ado-testcase.md`, reusing project
   terminology from the Knowledge Base. Preconditions and steps must use the real screens/routes
   and the configured test-account **roles** from the project context (e.g. "Signed in as
   <role>") instead of vague or invented references.
...
- Reference real routes, screens, and configured test-account roles from the Project Knowledge
  Base; **do not invent navigation or credentials**.
